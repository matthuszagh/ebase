...
def help():
    ...
    print("""Decrements the parts used in a project from the parts database. This is used when a
    project has been built and you wish to update the stock.\n""")
...
def main(argv, conn, cur):
    ...
    cur.execute(
        """insert into tmp select parts.mfn, stock - qty from parts, projects.{0} where parts.mfn = projects.{0}.mfn""".format(proj))
    cur.execute("""update tmp set stock=0 where stock<0""")
    cur.execute(
        """update parts set stock = tmp.stock from tmp where parts.mfn=tmp.mfn""")
